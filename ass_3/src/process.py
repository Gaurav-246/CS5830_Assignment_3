...
def compute_avg(csv_file, field_names):
    ...
    df = df[daily_field_names + monthly_field_names]                    # This DF contains just the required fields
    df = df.apply(pd.to_numeric, errors='coerce')                       # This DF changes string values of 'nan' to NaN
    
    avg_values_for_all_fields = []

    for i in range(len(field_names)):
        indices = [0]
        ground_truth_indices = df[monthly_field_names[i]][df[monthly_field_names[i]].notnull()].index       # Indices of monthly GT values
        indices += [index for index in ground_truth_indices]
        avg_value_for_this_field = []

        for j in range(len(indices)-1):                                 # Computes monthly avg from previous month GT index to current month GT index
            avg_value_for_this_field.append(np.nanmean(df[daily_field_names][indices[j]:indices[j+1]]))
...
def main():
    ...
    prepared_dir_path = sys.argv[2]
    computed_dir_path = '/home/laog/CS5830_Assignment_3/ass_3/computed'

    if not os.path.exists(computed_dir_path) :           # Creates the folder corresponding to the above path
    ...
    with open(fields_file, 'r') as file:
        ...
        for line in lines:
            ...
            items = line.split(',')
            items = [item.strip().strip("'\"") for item in items]  # Remove quotes and whitespace
